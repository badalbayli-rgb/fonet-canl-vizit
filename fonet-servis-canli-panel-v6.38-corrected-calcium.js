(() => {
  /********************************************************************
   * FONET SERVİS CANLI PANEL V6.38 DÜZELTİLMİŞ KALSİYUM
  ...
  function replacementViewHtml(patients = []) {
    ...
    const rows = ordered.map((p, index) => {
      ...
      const labCells = electrolytes.map((x) => {
        ...
        return `<div title="${escapeHtml(`${x.label}: ${x.displayValue || "sonuç yok"} · ${x.advice.title}`)}" style="height:38px;border:1px solid ${palette[1]}33;border-radius:7px;background:${palette[0]};padding:3px 4px;text-align:center;box-sizing:border-box;overflow:hidden;"><div style="color:${palette[1]};font-size:9px;font-weight:950;line-height:1;">${x.label}${arrow}</div><strong style="display:block;font-size:${x.key === "CaCorr" ? "11" : "13"}px;line-height:1.2;color:${t.text};">${escapeHtml(x.displayValue || "—")}</strong><small style="display:block;color:${t.muted};font-size:7px;line-height:1;">${escapeHtml(shortDate(x.date) || "yok")}</small></div>`;
      }).join("");
      const adviceRows = abnormal.map((x) => `<div style="border-left:4px solid ${x.advice.status === "high" ? "#dc2626" : "#f59e0b"};padding:6px 9px;background:${x.advice.status === "high" ? "#fef2f2" : "#fffbeb"};border-radius:6px;"><div style="display:flex;gap:7px;align-items:center;"><b>${x.adviceLabel} ${escapeHtml(x.value)}</b><span style="font-size:10px;font-weight:900;color:${x.advice.status === "high" ? "#b91c1c" : "#9a3412"};">${escapeHtml(x.advice.title)}</span><a href="${REPLACEMENT_SOURCES[x.key]}" target="_blank" rel="noreferrer" style="margin-left:auto;color:${t.primary2};font-size:10px;font-weight:800;">kaynak ↗</a></div><div style="font-size:11px;line-height:1.35;color:${t.text};margin-top:3px;">${escapeHtml(x.advice.text)}</div></div>`).join("");
    ...
    const header = `<div style="position:sticky;top:0;z-index:3;display:grid;grid-template-columns:${columns};gap:5px;align-items:center;min-width:790px;padding:5px 7px;background:${t.header};color:${t.headerText};border-radius:7px;font-size:8px;font-weight:950;text-transform:uppercase;letter-spacing:.03em;box-sizing:border-box;"><span>Hasta · FONET sırası</span>${["Na", "K", "Mg", "Ca/dCa", "P"].map((x) => `<span style="text-align:center;">${x}</span>`).join("")}<span>Replasman</span><span style="text-align:right;">Durum</span></div>`;
  ...
  function makePanel() {
    ...
    panel.innerHTML = `
      <header id="fsl-drag-handle" style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:12px;background:${t.header};color:${t.headerText};cursor:${state.popupMode ? "default" : "move"};user-select:none;border-bottom:1px solid ${t.border};">
        <div>
          <b>FONET Servis Canlı Panel V6.38 Düzeltilmiş Kalsiyum</b>
