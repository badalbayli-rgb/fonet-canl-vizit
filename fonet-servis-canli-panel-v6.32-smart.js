(() => {
  /********************************************************************
   * FONET SERVİS CANLI PANEL V6.32 SMART
  ...
  const state = {
    ...
    patientRefreshAt: {},
    endpointFailures: {},
    metrics: { cycles: 0, requests: 0, errors: 0, updated: 0, lastCycleMs: 0, lastCycleAt: 0, nextCycleAt: 0 },
    cardScroll: {},
    gridScroll: 0,
    radiologyTextCache: {},
...
<body></body>
  ...
  function criticalAlerts(p) {
    ...
    if (plt != null && plt < 50000) out.push(`PLT ${plt}`);
    return out;
  }
...
`.trim();
  ...
  async function refreshPatientDetails(p, shouldRender = true, forceAll = true) {
    p.loading = true;
    p.errors = [];
    ...
    const now = Date.now();
    const due = (label, interval) => forceAll || !stamps[label] || now - stamps[label] >= interval;
    const run = async (label, interval, fn) => {
      ...
      if (!forceAll && failure.retryAt > now) return;
      ...
      try {
        ...
        state.endpointFailures[failureKey] = { count, retryAt: Date.now() + Math.min(15 * 60000, count * 120000) };
        state.metrics.errors += 1;
      }
    };
    await run("Sevk", 600000, () => fetchSevkInfo(p));
    await run("Klinik", 600000, () => fetchClinical(p));
    const jobs = [
    ...
    for (const [label, interval, fn] of jobs) {
      await run(label, interval, fn);
    }
    p.loading = false;
  ...
  async function refreshAllDetails(force = false) {
    if (state.busy || !state.active || (!force && pageIsHidden())) return;
    state.busy = true;
    ...
    try {
      ...
      for (const p of candidates) {
        if (!state.active || (!force && pageIsHidden())) break;
        try { await refreshPatientDetails(p, false, false); state.metrics.updated += 1; } catch (e) {}
  ...
  function startAutoMonitor() {
    ...
    if (!state.visibilityHandler) {
      state.visibilityHandler = () => {
        if (!pageIsHidden() && state.active && state.monitor) refreshAllDetails(true);
      };
  ...
  function render() {
    ...
    if (endpoints) {
      ...
      endpoints.textContent = `Tur:${m.cycles} | Son:${m.lastCycleMs}ms | Sorgu:${m.requests} | Hata:${m.errors} | Güncel:${m.updated} | Sonraki:${nextSec}sn`;
    }
  ...
  function makePanel() {
    ...
    panel.innerHTML = `
      <header id="fsl-drag-handle" style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:12px;background:${t.header};color:${t.headerText};cursor:${state.popupMode ? "default" : "move"};user-select:none;border-bottom:1px solid ${t.border};">
        <div>
          <b>FONET Servis Canlı Panel V6.32 Smart</b>
  ...
  startAutoMonitor();
  window.setTimeout(() => {
    if (state.active) refreshAllDetails(true);
  }, 1500);
