(() => {
  /********************************************************************
   * FONET SERVİS CANLI PANEL V6.36 REPLASMAN TAKİBİ
...
<body></body>
  ...
  function replacementViewHtml(patients = []) {
    const t = activeTheme();
    const ordered = patients.slice().sort((a, b) => abnormalElectrolyteCount(b) - abnormalElectrolyteCount(a) || roomSort(a, b));
    ...
    const cards = ordered.map((p) => {
      ...
      const labCells = electrolytes.map((x) => {
        ...
        return `<div style="border:1px solid ${palette[1]}33;border-radius:8px;background:${palette[0]};padding:7px;text-align:center;min-width:72px;"><b style="display:block;color:${palette[1]};font-size:11px;">${x.key}</b><strong style="font-size:17px;color:${t.text};">${escapeHtml(x.value || "—")}</strong><small style="display:block;color:${t.muted};font-size:9px;">${escapeHtml(shortDate(x.date) || "sonuç yok")}</small></div>`;
      }).join("");
      ...
      return `<article data-replacement-card="${escapeHtml(key)}" style="border:1px solid ${done ? "#86bfa6" : t.border};border-left:5px solid ${abnormal.length ? "#f59e0b" : "#10b981"};border-radius:11px;background:${done ? "#f0fdf4" : t.surface};padding:11px;display:grid;gap:9px;box-shadow:${t.shadow};opacity:${done ? ".72" : "1"};">
        <div style="display:flex;align-items:center;gap:10px;"><button data-open-patient="${escapeHtml(key)}" style="border:0;background:transparent;color:${t.text};padding:0;text-align:left;cursor:pointer;"><b style="font-size:15px;">${escapeHtml(p.oda || "-")} · ${escapeHtml(p.adSoyad || "Hasta")}</b><small style="display:block;color:${t.muted};margin-top:2px;">${escapeHtml([p.birim, p.protokol].filter(Boolean).join(" | "))}</small></button><span style="margin-left:auto;border-radius:20px;padding:4px 8px;background:${abnormal.length ? "#fff7ed" : "#ecfdf5"};color:${abnormal.length ? "#c2410c" : "#047857"};font-size:10px;font-weight:900;">${abnormal.length ? `${abnormal.length} anormal` : "normal"}</span></div>
        <div style="display:grid;grid-template-columns:repeat(5,minmax(72px,1fr));gap:6px;">${labCells}</div>
        <div style="display:grid;gap:6px;">${adviceRows || `<div style="font-size:11px;color:#047857;background:#ecfdf5;border-radius:6px;padding:7px 9px;">Görünen son elektrolitlerde replasman gerektiren değer yok.</div>`}</div>
        <div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:7px;"><input data-replacement-note="${escapeHtml(key)}" value="${escapeHtml(state.replacementNotes?.[key] || "")}" placeholder="Manuel replasman / klinik not…" style="min-width:0;border:1px solid ${t.border};border-radius:7px;background:${t.surface};color:${t.text};padding:8px 10px;font-size:11px;"/><button data-replacement-done="${escapeHtml(key)}" style="border:0;border-radius:7px;background:${done ? "#059669" : "#475569"};color:white;padding:8px 10px;font-size:11px;font-weight:900;cursor:pointer;">${done ? "✓ Kontrol edildi" : "Kontrol edildi"}</button></div>
      </article>`;
    }).join("");
    return `<section style="grid-column:1/-1;border:1px solid ${t.border};border-radius:11px;background:${t.surface2};padding:11px;display:grid;gap:9px;"><div style="display:flex;align-items:center;gap:12px;"><div><b style="font-size:17px;color:${t.text};">Elektrolit Replasman Takibi</b><div style="font-size:11px;color:${t.muted};margin-top:2px;">${ordered.length} hasta · ${abnormalPatients} hastada anormal elektrolit</div></div><button id="fsl-replacement-refresh" style="margin-left:auto;border:0;border-radius:7px;background:${t.primary};color:white;padding:8px 11px;font-weight:900;cursor:pointer;">Sonuçları yenile</button></div><div style="font-size:10px;color:#7c2d12;background:#fff7ed;border:1px solid #fed7aa;border-radius:7px;padding:7px 9px;">Bilgilendirme amaçlıdır; order değildir. Semptom, EKG, eGFR/diürez, ilaçlar, oral alım, devam eden kayıp ve kurum protokolü doğrulanmadan uygulamayın.</div></section>${cards || `<div style="grid-column:1/-1;padding:24px;text-align:center;color:${t.muted};">Hasta veya laboratuvar sonucu bulunamadı.</div>`}`;
  }
...
`.trim();
  ...
  function render() {
    ...
    grid.style.gridTemplateColumns = state.activeView === "replacement" ? "1fr" : `repeat(auto-fill,minmax(${state.cardWidth || 360}px,1fr))`;
  ...
  function makePanel() {
    ...
    panel.innerHTML = `
      <header id="fsl-drag-handle" style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:12px;background:${t.header};color:${t.headerText};cursor:${state.popupMode ? "default" : "move"};user-select:none;border-bottom:1px solid ${t.border};">
        <div>
          <b>FONET Servis Canlı Panel V6.36 Replasman Takibi</b>
