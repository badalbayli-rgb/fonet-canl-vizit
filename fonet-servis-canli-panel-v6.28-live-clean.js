...
<body></body>
  ...
  function collectServicePatients() {
  ...
  function addRequest(req) {
    ...
    if (type === "patient") collectServicePatients();
    if (appliedPatientDetail || appliedRadiologyReport || appliedAnesthesia) render();
    else render();
  }
...
`.trim();
  ...
  async function refreshPatientDetails(p) {
    p.loading = true;
    p.errors = [];
    render();
  ...
  function makePanel() {
    ...
    uiEl("fsl-watch").onclick = () => {
      ...
      refreshAllDetails();
      state.lastMessage = "Otomatik servis izleme açık: 60 sn.";
      render();
    };
