...
`.trim();
  ...
  function startAutoMonitor() {
    if (!state.monitor) {
      state.monitor = window.setInterval(refreshAllDetails, 120000);
    }
    if (!state.visibilityHandler) {
      state.visibilityHandler = () => {
        if (!document.hidden && state.active && state.monitor) refreshAllDetails();
      };
  ...
  startAutoMonitor();
  window.setTimeout(() => {
    if (state.active && !document.hidden) refreshAllDetails();
  }, 1500);
