(() => {
  /********************************************************************
   * FONET SERVİS CANLI PANEL V6.37 KOMPAKT REPLASMAN LİSTESİ
  ...
  const state = {
    ...
    panelWindow: null,
    popupMode: false,
    cardWidth: (() => { try { return Number(localStorage.getItem("fslCardWidth")) || 380; } catch (e) { return 380; } })(),
    cardHeight: (() => { try { return Number(localStorage.getItem("fslCardHeight")) || 248; } catch (e) { return 248; } })(),
    cardOrder: [],
    fonetOrder: [],
    sortMode: (() => { try { return localStorage.getItem("fslSortMode") || "fonet"; } catch (e) { return "fonet"; } })(),
    pinnedKeys: (() => { try { return JSON.parse(localStorage.getItem("fslPinnedKeys") || "[]"); } catch (e) { return []; } })(),
    pausedKeys: (() => { try { return JSON.parse(localStorage.getItem("fslPausedKeys") || "[]"); } catch (e) { return []; } })(),
    thresholds: (() => { try { return { tempHigh:38, spo2Low:92, pulseLow:50, pulseHigh:120, sysLow:90, sysHigh:180, glucoseLow:70, glucoseHigh:250, potassiumLow:3, potassiumHigh:6, ...(JSON.parse(localStorage.getItem("fslThresholds") || "{}")) }; } catch (e) { return { tempHigh:38, spo2Low:92, pulseLow:50, pulseHigh:120, sysLow:90, sysHigh:180, glucoseLow:70, glucoseHigh:250, potassiumLow:3, potassiumHigh:6 }; } })(),
    patientRefreshAt: {},
    endpointFailures: {},
    ...
    gridScroll: 0,
    activeView: "patients",
    replacementNotes: (() => { try { const v = JSON.parse(localStorage.getItem("fslReplacementNotes") || "{}"); return v && typeof v === "object" ? v : {}; } catch (e) { return {}; } })(),
    replacementDone: (() => { try { const v = JSON.parse(localStorage.getItem("fslReplacementDone") || "{}"); return v && typeof v === "object" ? v : {}; } catch (e) { return {}; } })(),
    radiologyTextCache: {},
...
<body></body>
  ...
  function saveSmartSettings() {
    try {
      localStorage.setItem("fslSortMode", state.sortMode);
      localStorage.setItem("fslPinnedKeys", JSON.stringify(state.pinnedKeys || []));
      localStorage.setItem("fslPausedKeys", JSON.stringify(state.pausedKeys || []));
      localStorage.setItem("fslThresholds", JSON.stringify(state.thresholds || {}));
      localStorage.setItem("fslCardWidth", String(state.cardWidth || 380));
      localStorage.setItem("fslCardHeight", String(state.cardHeight || 248));
      localStorage.setItem("fslReplacementNotes", JSON.stringify(state.replacementNotes || {}));
      localStorage.setItem("fslReplacementDone", JSON.stringify(state.replacementDone || {}));
    } catch (e) {}
  }
  ...
  function replacementAdvice(key, value) {
    ...
    if (key === "Ca") return { status, title: "Düşük", text: "Total Ca ile doz seçme. Önce albümine göre düzeltilmiş veya iyonize Ca, Mg, semptom ve EKG; Mg düşüklüğünü önce düzelt. Doz/yol hasta özelinde belirlenir." };
    if (key === "K") return n < 3
  ...
  function patientElectrolytes(p) {
    return ["Na", "K", "Mg", "Ca", "P"].map((key) => {
      const item = p?.labs?.[key]?.[0] || {};
      return { key, value: item.value || "", date: item.date || "", advice: replacementAdvice(key, item.value) };
    });
  }
  ...
  function compactReplacementText(item) {
    ...
    if (item.advice.status === "high") return `${item.key}: replasman verme`;
    ...
    if (item.key === "Ca") return "Ca: düzeltilmiş/iyonize Ca sonrası karar";
  ...
  function replacementViewHtml(patients = []) {
    ...
    const columns = "minmax(185px,1.3fr) repeat(5,minmax(54px,64px)) minmax(210px,1.7fr) 78px";
    const rows = ordered.map((p, index) => {
      ...
      const labCells = electrolytes.map((x) => {
        ...
        return `<div title="${escapeHtml(`${x.key}: ${x.value || "sonuç yok"} · ${x.advice.title}`)}" style="height:38px;border:1px solid ${palette[1]}33;border-radius:7px;background:${palette[0]};padding:3px 4px;text-align:center;box-sizing:border-box;overflow:hidden;"><div style="color:${palette[1]};font-size:9px;font-weight:950;line-height:1;">${x.key}${arrow}</div><strong style="display:block;font-size:13px;line-height:1.2;color:${t.text};">${escapeHtml(x.value || "—")}</strong><small style="display:block;color:${t.muted};font-size:7px;line-height:1;">${escapeHtml(shortDate(x.date) || "yok")}</small></div>`;
      }).join("");
      const adviceRows = abnormal.map((x) => `<div style="border-left:4px solid ${x.advice.status === "high" ? "#dc2626" : "#f59e0b"};padding:6px 9px;background:${x.advice.status === "high" ? "#fef2f2" : "#fffbeb"};border-radius:6px;"><div style="display:flex;gap:7px;align-items:center;"><b>${x.key} ${escapeHtml(x.value)}</b><span style="font-size:10px;font-weight:900;color:${x.advice.status === "high" ? "#b91c1c" : "#9a3412"};">${escapeHtml(x.advice.title)}</span><a href="${REPLACEMENT_SOURCES[x.key]}" target="_blank" rel="noreferrer" style="margin-left:auto;color:${t.primary2};font-size:10px;font-weight:800;">kaynak ↗</a></div><div style="font-size:11px;line-height:1.35;color:${t.text};margin-top:3px;">${escapeHtml(x.advice.text)}</div></div>`).join("");
      const replacementText = abnormal.map(compactReplacementText).filter(Boolean).join(" • ") || "Replasman görünmüyor";
      return `<article data-replacement-row="${escapeHtml(key)}" style="border:1px solid ${done ? "#86bfa6" : t.border};border-left:4px solid ${abnormal.length ? "#f59e0b" : "#10b981"};border-radius:8px;background:${done ? "#f0fdf4" : t.surface};overflow:hidden;opacity:${done ? ".76" : "1"};">
        <details>
          <summary title="Öneri ve kaynakları aç" style="list-style:none;display:grid;grid-template-columns:${columns};gap:5px;align-items:center;min-width:790px;min-height:46px;padding:4px 7px;box-sizing:border-box;cursor:pointer;user-select:none;">
            <div style="min-width:0;display:grid;grid-template-columns:25px minmax(0,1fr);gap:5px;align-items:center;"><span style="width:23px;height:23px;border-radius:7px;display:grid;place-items:center;background:${abnormal.length ? "#fff7ed" : "#ecfdf5"};color:${abnormal.length ? "#c2410c" : "#047857"};font-size:9px;font-weight:950;">${index + 1}</span><span style="min-width:0;"><b style="display:block;color:${t.text};font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(p.oda || "-")} · ${escapeHtml(p.adSoyad || "Hasta")}</b><small style="display:block;color:${t.muted};font-size:8px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml([p.birim, p.protokol].filter(Boolean).join(" | "))}</small></span></div>
            ${labCells}
            <div title="${escapeHtml(replacementText)}" style="min-width:0;color:${abnormal.length ? "#9a3412" : "#047857"};font-size:10px;font-weight:${abnormal.length ? "850" : "750"};line-height:1.2;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">${escapeHtml(replacementText)}</div>
            <span style="justify-self:end;border-radius:20px;padding:4px 7px;background:${abnormal.length ? "#fff7ed" : "#ecfdf5"};color:${abnormal.length ? "#c2410c" : "#047857"};font-size:8px;font-weight:950;white-space:nowrap;">${done ? "✓ " : ""}${abnormal.length ? `${abnormal.length} anormal ›` : "normal ›"}</span>
          </summary>
          <div style="border-top:1px solid ${t.border};background:${t.surface2};padding:7px 9px 8px;display:grid;gap:6px;">
            <div style="font-size:10px;font-weight:950;color:${t.primary2};">Öneri ve kaynaklar</div>
            <div style="display:grid;gap:5px;">${adviceRows || `<div style="font-size:10px;color:#047857;background:#ecfdf5;border-radius:6px;padding:6px 8px;">Görünen son elektrolitlerde replasman gerektiren değer yok.</div>`}</div>
            <div style="display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:6px;"><input data-replacement-note="${escapeHtml(key)}" value="${escapeHtml(state.replacementNotes?.[key] || "")}" placeholder="Manuel replasman / klinik not…" style="min-width:0;border:1px solid ${t.border};border-radius:7px;background:${t.surface};color:${t.text};padding:6px 8px;font-size:10px;"/><button data-open-patient="${escapeHtml(key)}" style="border:1px solid ${t.border};border-radius:7px;background:${t.surface};color:${t.primary2};padding:6px 9px;font-size:10px;font-weight:900;cursor:pointer;">Hasta kartı</button><button data-replacement-done="${escapeHtml(key)}" style="border:0;border-radius:7px;background:${done ? "#059669" : "#475569"};color:white;padding:6px 9px;font-size:10px;font-weight:900;cursor:pointer;">${done ? "✓ Kontrol edildi" : "Kontrol edildi"}</button></div>
          </div>
        </details>
      </article>`;
    }).join("");
    const header = `<div style="position:sticky;top:0;z-index:3;display:grid;grid-template-columns:${columns};gap:5px;align-items:center;min-width:790px;padding:5px 7px;background:${t.header};color:${t.headerText};border-radius:7px;font-size:8px;font-weight:950;text-transform:uppercase;letter-spacing:.03em;box-sizing:border-box;"><span>Hasta · FONET sırası</span>${["Na", "K", "Mg", "Ca", "P"].map((x) => `<span style="text-align:center;">${x}</span>`).join("")}<span>Replasman</span><span style="text-align:right;">Durum</span></div>`;
    return `<section style="grid-column:1/-1;border:1px solid ${t.border};border-radius:9px;background:${t.surface2};padding:7px 9px;display:flex;align-items:center;gap:10px;"><div style="min-width:0;"><b style="font-size:14px;color:${t.text};">Elektrolit Replasman Takibi</b><span style="font-size:10px;color:${t.muted};margin-left:8px;">${ordered.length} hasta · ${abnormalPatients} hastada anormal değer · FONET sırası</span><div style="font-size:8px;color:#9a3412;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">Satırda replasman özeti görünür; hastaya tıklayınca kaynaklı öneri açılır. Bilgilendirme amaçlıdır, order değildir.</div></div><button id="fsl-replacement-refresh" style="margin-left:auto;border:0;border-radius:7px;background:${t.primary};color:white;padding:6px 9px;font-size:10px;font-weight:900;cursor:pointer;white-space:nowrap;">Sonuçları yenile</button></section><div style="grid-column:1/-1;display:grid;gap:4px;min-width:0;overflow-x:auto;">${header}${rows || `<div style="padding:24px;text-align:center;color:${t.muted};">Hasta veya laboratuvar sonucu bulunamadı.</div>`}</div>`;
  }
...
`.trim();
  ...
  function render() {
    ...
    grid.style.gridTemplateColumns = replacementMode ? "1fr" : `repeat(auto-fill,minmax(${state.cardWidth || 360}px,1fr))`;
    grid.innerHTML = state.activeView === "replacement"
  ...
  function makePanel() {
    ...
    panel.innerHTML = `
      <header id="fsl-drag-handle" style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:12px;background:${t.header};color:${t.headerText};cursor:${state.popupMode ? "default" : "move"};user-select:none;border-bottom:1px solid ${t.border};">
        <div>
          <b>FONET Servis Canlı Panel V6.37 Kompakt Replasman</b>
        ...
        <div style="display:flex;gap:7px;align-items:center;flex-wrap:wrap;justify-content:flex-end;">
          ...
          <select id="fsl-sort-mode" title="Kart sıralaması; varsayılan FONET sırasıdır" style="height:32px;border:1px solid ${t.border};border-radius:8px;background:${t.surface};color:${t.text};padding:0 8px;font-weight:800;cursor:pointer;">
            <option value="fonet"${state.sortMode === "fonet" ? " selected" : ""}>FONET Sırası</option>
            <option value="priority"${state.sortMode === "priority" ? " selected" : ""}>Kritikler Önce</option>
            <option value="room"${state.sortMode === "room" ? " selected" : ""}>Oda/Yatak</option>
            <option value="name"${state.sortMode === "name" ? " selected" : ""}>İsme Göre</option>
          </select>
          <button id="fsl-critical-settings" title="Kritik vital sınırları" style="background:#b91c1c;color:white;border:0;border-radius:6px;padding:7px 10px;cursor:pointer;">Kritik Ayar</button>
      ...
      <section style="display:grid;grid-template-columns:minmax(220px,1fr) auto;gap:9px;align-items:center;padding:9px 11px;background:${t.surface2};border-bottom:1px solid ${t.border};">
        ...
        <div style="display:flex;gap:6px;align-items:center;">
          <button id="fsl-view-patients" style="height:38px;border:0;border-radius:9px;background:${state.activeView === "patients" ? t.primary : "#64748b"};color:white;padding:0 13px;font-weight:950;cursor:pointer;white-space:nowrap;">Vizit</button>
          <button id="fsl-view-replacement" style="height:38px;border:0;border-radius:9px;background:${state.activeView === "replacement" ? "#d97706" : "#64748b"};color:white;padding:0 13px;font-weight:950;cursor:pointer;white-space:nowrap;">Replasman</button>
