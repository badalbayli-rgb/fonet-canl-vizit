(() => {
  /********************************************************************
   * FONET SERVİS CANLI PANEL V6.33 RELIABLE
...
`.trim();
  ...
  async function refreshPatientDetails(p, shouldRender = true, forceAll = true, onlyLabels = null) {
    ...
    const run = async (label, interval, fn) => {
      ...
      try {
        await Promise.race([
          fn(),
          new Promise((_, reject) => window.setTimeout(() => reject(new Error("15 sn zaman aşımı")), 15000))
        ]);
  ...
  async function refreshAllDetails(force = false) {
    ...
    try {
      ...
      for (const p of candidates) {
        ...
        try {
          const failed = await refreshPatientDetails(p, false, false);
        ...
        if (state.metrics.processed % 5 === 0 || state.metrics.processed === state.metrics.total) scheduleRender(200);
  ...
  function makePanel() {
    ...
    panel.innerHTML = `
      <header id="fsl-drag-handle" style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:12px;background:${t.header};color:${t.headerText};cursor:${state.popupMode ? "default" : "move"};user-select:none;border-bottom:1px solid ${t.border};">
        <div>
          <b>FONET Servis Canlı Panel V6.33 Reliable</b>
  ...
  startAutoMonitor();
  window.setTimeout(() => {
    if (state.active) collectServicePatients(false);
  }, 2500);
  window.setTimeout(() => {
