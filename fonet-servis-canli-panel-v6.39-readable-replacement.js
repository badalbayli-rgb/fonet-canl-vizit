(() => {
  /********************************************************************
   * FONET SERVİS CANLI PANEL V6.39 OKUNAKLI REPLASMAN
  ...
  function replacementViewHtml(patients = []) {
    ...
    const rows = ordered.map((p, index) => {
      ...
      const labCells = electrolytes.map((x) => {
        ...
        return `<div title="${escapeHtml(`${x.label}: ${x.displayValue || "sonuç yok"} · ${x.advice.title}`)}" style="height:44px;border:1px solid ${palette[1]}55;border-radius:7px;background:${palette[0]};padding:4px 5px;text-align:center;box-sizing:border-box;overflow:hidden;"><div style="color:${palette[1]};font-size:11px;font-weight:950;line-height:1;">${x.label}${arrow}</div><strong style="display:block;font-size:${x.key === "CaCorr" ? "13" : "16"}px;line-height:1.15;color:${t.text};">${escapeHtml(x.displayValue || "—")}</strong><small style="display:block;color:#475569;font-size:9px;font-weight:700;line-height:1;">${escapeHtml(shortDate(x.date) || "yok")}</small></div>`;
      }).join("");
      ...
      return `<article data-replacement-row="${escapeHtml(key)}" style="border:1px solid ${done ? "#86bfa6" : "#b8c5d6"};border-left:5px solid ${abnormal.length ? "#f59e0b" : "#10b981"};border-radius:8px;background:${done ? "#f0fdf4" : (index % 2 ? t.surface2 : t.surface)};overflow:hidden;opacity:${done ? ".8" : "1"};">
        <details>
          <summary title="Öneri ve kaynakları aç" style="list-style:none;display:grid;grid-template-columns:${columns};gap:6px;align-items:center;min-width:900px;min-height:54px;padding:5px 8px;box-sizing:border-box;cursor:pointer;user-select:none;">
    ...
    const header = `<div style="position:sticky;top:0;z-index:3;display:grid;grid-template-columns:${columns};gap:6px;align-items:center;min-width:900px;padding:7px 8px;background:${t.header};color:${t.headerText};border-radius:7px;font-size:10px;font-weight:950;text-transform:uppercase;letter-spacing:.035em;box-sizing:border-box;"><span>Hasta · FONET sırası</span>${["Na", "K", "Mg", "Ca/dCa", "P"].map((x) => `<span style="text-align:center;">${x}</span>`).join("")}<span>Replasman</span><span style="text-align:right;">Durum</span></div>`;
    return `<section style="grid-column:1/-1;border:1px solid ${t.border};border-radius:9px;background:${t.surface2};padding:8px 10px;display:flex;align-items:center;gap:10px;"><div style="min-width:0;"><b style="font-size:16px;color:${t.text};">Elektrolit Replasman Takibi</b><span style="font-size:11px;color:#475569;margin-left:8px;font-weight:700;">${ordered.length} hasta · ${abnormalPatients} hastada anormal değer · FONET sırası</span><div style="font-size:10px;color:#9a3412;font-weight:650;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">Satırda replasman özeti görünür; hastaya tıklayınca kaynaklı öneri açılır. Bilgilendirme amaçlıdır, order değildir.</div></div><button id="fsl-replacement-refresh" style="margin-left:auto;border:0;border-radius:7px;background:${t.primary};color:white;padding:8px 11px;font-size:12px;font-weight:900;cursor:pointer;white-space:nowrap;">Sonuçları yenile</button></section><div style="grid-column:1/-1;display:grid;gap:5px;min-width:0;overflow-x:auto;">${header}${rows || `<div style="padding:24px;text-align:center;color:${t.muted};">Hasta veya laboratuvar sonucu bulunamadı.</div>`}</div>`;
  }
  ...
  function makePanel() {
    ...
    panel.innerHTML = `
      <header id="fsl-drag-handle" style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:12px;background:${t.header};color:${t.headerText};cursor:${state.popupMode ? "default" : "move"};user-select:none;border-bottom:1px solid ${t.border};">
        <div>
          <b>FONET Servis Canlı Panel V6.39 Okunaklı Replasman</b>
